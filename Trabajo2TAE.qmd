...
```{r}
#Si se deseea guardar el modelo, se ejecuta la seiguiente linea
...
pruebaPepe<-data.frame(int_rate,installment,annual_inc,open_acc,verification_status,purpose)
pepe2 <- datos_sel[1,]
pepe2 <- subset(pepe2, select = -c(loan_status,term))
valor <- scorecard_ply(pruebaPepe, card_nuevo)
```

```{r}
#select()
#select(dataos_selec2["other",6]
#vector<- 1:73408
#dataos_selec2$id <- vector       
#filtro <-dataos_selec2[dataos_selec2$purpose=="renewable_energy"]

#dataos_selec2 <- subset(dataos_selec2, select = -c(id))
```
